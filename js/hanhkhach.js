document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateCountries() {
        ...
        countrySelect.innerHTML = rawOptions;
        
        Array.from(countrySelect.options).forEach(opt => {
            if(opt.value === "84") opt.selected = true;
        });
    }
    ...
    const nameInputs = document.querySelectorAll('input[placeholder="Tên đệm và tên"], input[placeholder="Họ"]');
    ...
    btnConfirm.addEventListener('click', function(e) {
        ...
        if (!title || title === "Chọn danh xưng") return alert("Vui lòng chọn danh xưng!");
        if (firstName.length < 2 || lastName.length < 2) return alert("Họ tên quá ngắn hoặc không hợp lệ!");
        if (phone.length < 8 || isNaN(phone)) return alert("Số điện thoại phải là số!");
        if (!agree) return alert("Bạn chưa đồng ý với điều khoản!");

        const info = {
            name: `${lastName} ${firstName}`,
            phone: `+${countrySelect.value} ${phone}`,
